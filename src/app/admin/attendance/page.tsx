  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
        {profile.role === "teacher" ? <AttendanceRoster students={(students || []) as any} marked={markedMap} classId={classId} date={date} canEdit /> : <div className="card"><h2 className="card-title mb-2">Principal monitoring view</h2><p className="text-sm text-slate-500">Assigned teachers record attendance. Use the filters and report table to monitor class and student totals.</p></div>}
        <div className="card overflow-x-auto"><h2 className="card-title mb-3">Student-wise report</h2><p className="mb-4 text-xs text-slate-500">{fromDate} to {toDate}</p><table className="table"><thead><tr><th>Student</th><th>P</th><th>A</th><th>%</th></tr></thead><tbody>{(students || []).map((student) => { const item = studentTotals.get(student.id) || { present: 0, absent: 0 }; const count = item.present + item.absent; return <tr key={student.id}><td className="font-medium">{student.profiles?.full_name || "-"}</td><td><Badge color="green">{item.present}</Badge></td><td><Badge color="red">{item.absent}</Badge></td><td>{count ? `${Math.round((item.present / count) * 100)}%` : "-"}</td></tr>; })}</tbody></table>{!students?.length && <p className="py-8 text-center text-sm text-slate-400">No students in this class.</p>}</div>
      </div>
    </>
  );
}
